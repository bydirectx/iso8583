...
public final class HexCodec {
	...
	public static String toEvenHex(byte[] bytes) {
		String hex = toHex(bytes);
		if (hex != null && hex.length() % 2 != 0) hex = "0" + hex;
		return hex;
	}
	...
	public static String toHex(byte[] bytes) {
		if (bytes != null) {
			...
			for (byte aByte : bytes) {
				int byteValue = toUnsignedInt(aByte);
				String strValue = Integer.toHexString(byteValue);

				if (strValue.length() == 1 && sb.length() != 0) {
					sb.append("0");
				}
	...
	public static byte[] hexDecodeLlnum(String hex) {
    	hex = hex.length() % 2 == 0 ? hex : hex+"0";
